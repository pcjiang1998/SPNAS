...
def set_dataset(dataset_name):
    ...
    if dataset_name == 'NB201':
        benchmark = build_benchmark(dataset_name)
    elif dataset_name == 'NB201-Cifar100':
        benchmark = build_benchmark(dataset_name)
    elif dataset_name == 'NB201-ImageNet16-120':
        benchmark = build_benchmark(dataset_name)
    else:
        raise Exception()
    name = dataset_name
...
def surrogate(x, model, need_train, true_label=None):
    ...
    x = np.eye(5)[x].reshape([x.shape[0], -1])
    ...
    if need_train:
        ...
        history_data = [(np.eye(5)[item.original_encode].reshape([-1]), item.data.item())
