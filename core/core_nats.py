...
def set_dataset(dataset_name):
    global name, benchmark
    if dataset_name == 'NATS':
        benchmark = build_benchmark(dataset_name)
    elif dataset_name == 'NATS-Cifar100':
        benchmark = build_benchmark(dataset_name)
    elif dataset_name == 'NATS-ImageNet16-120':
        benchmark = build_benchmark(dataset_name)
    else:
        raise Exception()
    name = dataset_name
...
def surrogate(x, model, need_train, true_label=None):
    ...
    x = np.eye(8)[x].reshape([x.shape[0], -1])
    ...
    if need_train:
        ...
        history_data = [(np.eye(8)[item.original_encode].reshape([-1]), item.data.item())
                        for item in callback.history.values()]
        # history_data = [(item.original_encode, item.data.item())
        #                 for item in callback.history.values()]
